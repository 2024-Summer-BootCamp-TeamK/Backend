...
def delete_expired_files():
    ...
    logger.debug(f"Found {expired_documents.count()} expired documents.")
    for document in expired_documents:
        logger.debug(f"Expired document id={document.id}, pdfUrl={document.pdfUrl.name}")
        ...
        try:
            ...
            document.delete()
            logger.info(f"Deleted document {document.id} from database.")
        except Exception as e:
            logger.error(f"Failed to delete {document.pdfUrl.name}: {e}")
